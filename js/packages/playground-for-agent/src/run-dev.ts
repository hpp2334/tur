...
function rectCenterContainsPoint(r: Rect, px: number, py: number): boolean {
...
function findTextSpans(elements: ElementInfo[], text: string): ElementInfo[] {
  return findAll(elements, (e) => e.label === text && e.type === "tur_text_span");
}
...
function findSmallestContaining(
  ...
  const candidates = findAll(elements, (e) => e.type === type && contains(e.rect, innerRect));
  candidates.sort((a, b) => (a.rect.w * a.rect.h) - (b.rect.w * b.rect.h));
  return candidates[0];
}

function findModalAddTaskButton(elements: ElementInfo[]): { x: number; y: number } | null {
  const spans = findTextSpans(elements, "Add Task");
  for (const span of spans) {
    const btn = findSmallestContaining(elements, span.rect, "tur_pointer_interact");
...
function findHeaderNewTaskButton(elements: ElementInfo[]): { x: number; y: number } | null {
...
function findModalInput(elements: ElementInfo[], index: number = 0): ElementInfo | undefined {
  const inputs = findAll(elements, (e) => e.type === "tur_input" && e.rect.w > 300 && e.rect.h < 50);
  return inputs[index];
}

function findTaskListInput(elements: ElementInfo[]): ElementInfo | undefined {
  const inputs = findAll(elements, (e) => e.type === "tur_input" && e.rect.w > 500);
  ...
  for (const span of spans) {
    ...
    const checkbox = findElement(
      ...
      (e) =>
        ...
        e.rect.w <= 30 &&
        e.rect.h <= 30 &&
        Math.abs(e.rect.y + e.rect.h / 2 - (sy + span.rect.h / 2)) < 20 &&
...
function findItemRow(
  ...
  const spans = findTextSpans(elements, itemText);
  for (const span of spans) {
    const row = findSmallestContaining(elements, span.rect, "tur_pointer_interact");
...
function hasModalOpen(elements: ElementInfo[]): boolean {
  return elements.some((e) => e.label === "Add New Task" && e.type === "tur_text_span");
}
...
function getTaskTexts(elements: ElementInfo[]): string[] {
  ...
  for (const e of elements) {
    if (
      ...
      const hasDeleteBtn = elements.some((d) =>
      ...
      const hasCheckbox = elements.some((c) =>
...
async function getLayout(page: Page): Promise<ElementInfo[]> {
  ...
  console.log(`  Parsed ${elements.length} elements (${(layout || "").length} chars)`);
  return elements;
}

async function assertClick(page: Page, elements: ElementInfo[], label: string, target: { x: number; y: number } | null) {
  if (!target) {
...
async function main() {
  ...
  try {
    ...
    if (initialTasks.length >= 4) {
      ...
      console.log(`  FAIL: expected >=4 tasks, got ${initialTasks.length}`);
      failed++;
    }
    ...
    if (modalInput) {
      console.log(`  PASS: found modal title input at (${modalInput.rect.x}, ${modalInput.rect.y}) ${modalInput.rect.w}x${modalInput.rect.h}`);
      passed++;
    } else {
    ...
    elements = await getLayout(page);
    const typedInput = findElement(elements, (e) => e.type === "tur_input" && e.label.includes("Buy groceries"));
    if (typedInput) {
      console.log(`  PASS: input shows "${typedInput.label}"`);
      passed++;
    } else {
      const allInputs = findAll(elements, (e) => e.type === "tur_input");
      console.log(`  FAIL: input does not show "Buy groceries". Inputs: ${allInputs.map((e) => `"${e.label}"`).join(", ")}`);
      failed++;
    }
    ...
    console.log(`  Tasks after second add: ${taskTextsAfterAdd2.join(", ")}`);
    if (!hasModalOpen(elements)) {
    ...
    await assertClick(page, elements, "checkbox for Build tur engine", checkbox);
    ...
    if (buildTurSpan.length > 0) {
      ...
      const checkmark = findElement(
        ...
        (e) =>
          e.type === "tur_text_span" &&
          e.label === "v" &&
          Math.abs(e.rect.y + e.rect.h / 2 - (sy + buildTurSpan[0].rect.h / 2)) < 20 &&
    ...
    console.log("\n--- Step 7: Delete 'Write documentation' ---");
    elements = await getLayout(page);
    const deleteBtn = findDeleteButtonForItem(elements, "Write documentation");
    await assertClick(page, elements, "delete for Write documentation", deleteBtn);
    ...
    if (!taskTextsAfterDelete.includes("Write documentation")) {
      ...
      console.log("  FAIL: 'Write documentation' still present after delete");
      failed++;
    }
    ...
    const expectedTasks = ["Learn Rust", "Build tur engine", "Ship v0.1.0", "Buy groceries", "Review PR"];
    const allExpectedPresent = expectedTasks.every((t) => finalTasks.includes(t));
    const writeDocGone = !finalTasks.includes("Write documentation");
    if (allExpectedPresent && writeDocGone) {
      console.log(`  PASS: correct tasks present (${finalTasks.length} items including detail panel texts)`);
      passed++;
    } else {
      const missing = expectedTasks.filter((t) => !finalTasks.includes(t));
      const unexpected = finalTasks.filter((t) => !expectedTasks.includes(t) && t !== "Write documentation");
      console.log(`  FAIL: missing=${missing.join(",")} unexpected=${unexpected.join(",")}`);
      failed++;
    }

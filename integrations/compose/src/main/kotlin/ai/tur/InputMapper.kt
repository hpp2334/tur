 ...
 * `key` / `code` strings the engine expects (it was designed against DOM
 * `KeyboardEvent.key` / `.code`, so the playground's key handling assumes those
 * semantics — e.g. `"Enter"`, `"Backspace"`, `"ArrowLeft"`, `"KeyA"`).
 *
 * The `code` column mirrors the physical-key `KeyboardEvent.code` layout
 * (`KeyA`, `Digit1`, `ArrowLeft`, …). The `key` column is the logical key
 * name. Modifiers and ASCII printable chars are derived from the event itself.
 */
object InputMapper {
    ...
    fun map(keyCode: Int): Pair<String, String>? = when (keyCode) {
        ...
        // Modifiers (sent so the engine sees modifier key presses too)

...
//! `transpileTsx`, `tokenizeTsx`, `generateAst`. ctx-free fn pointers
//! registered by `TurPlaygroundPlugin` as part of the `tur-ext/demo-helper`
//! module.
...
fn tur_transpile_tsx(_this: &JsValue, args: &[JsValue], _ctx: &mut Context) -> JsResult<JsValue> {
    let src = args
        .get_or_undefined(0)
        .as_string()
        .ok_or_else(|| {
...
fn tur_tokenize_tsx(_this: &JsValue, args: &[JsValue], ctx: &mut Context) -> JsResult<JsValue> {
    let src = args
        .get_or_undefined(0)
        .as_string()
        .ok_or_else(|| {
...
fn tur_generate_ast(_this: &JsValue, args: &[JsValue], ctx: &mut Context) -> JsResult<JsValue> {
    let src = args
        .get_or_undefined(0)
        .as_string()
        .ok_or_else(|| {
...
pub fn build_host_service_fns() -> Vec<(&'static str, boa_engine::NativeFunction, usize)> {
    vec![
        (
            "transpileTsx",
            NativeFunction::from_fn_ptr(tur_transpile_tsx),
            1,
        ),
        (
            "tokenizeTsx",
            NativeFunction::from_fn_ptr(tur_tokenize_tsx),
            1,
        ),
        (
            "generateAst",
            NativeFunction::from_fn_ptr(tur_generate_ast),
            1,
        ),
    ]
}

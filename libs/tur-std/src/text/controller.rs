...
use tur_engine::core::edgy_event::{extract_mutation_from_opts, EdgyMutation};
...
use crate::text::span_data::SpanData;
...
impl TextEditingController {
    ...
    fn maybe_push_undo(&mut self) {
        ...
        let snapshot = crate::text::TextEditingValue {
            text: self.text(),
            cursor_position: self.cursor_position,
            selection_anchor: self.selection_anchor,
            selection_end: self.selection_end,
        };
        if let Some(mut undo) = recorder.downcast_mut::<crate::text::UndoController>() {
            undo.push(snapshot);
        }
    }
...
impl Class for TextEditingController {
    ...
    fn data_constructor(
        ...
        if let Some(opts) = args.get_or_undefined(0).as_object() {
            ...
            {
                if !initial.is_empty() {
                    ctrl.set_spans(vec![crate::text::span_data::SpanData {
                        text: initial,
                        bold: false,
                        italic: false,
                        underline: false,
    ...
    fn init(class: &mut ClassBuilder<'_>) -> JsResult<()> {
        ...
        class.method(
            ...
            NativeFunction::from_fn_ptr(|this, args, ctx| {
                ...
                let spans = crate::text::span_data::extract_spans_from_js(
        ...
        class.method(
            ...
            NativeFunction::from_fn_ptr(|this, args, _ctx| {
                ...
                let recorder = match args.get_or_undefined(0).as_object() {
                    Some(o) if o.downcast_ref::<crate::text::UndoController>().is_some() => {
                        Some(o.clone())
                    }
                    _ => None,
                };

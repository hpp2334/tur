...
/// canvas; tests record it for assertions. The engine owns a `Box<dyn PlatformApi>`
...
/// Convenience `PlatformApi` that discards everything. Used by embedders/tests
/// that don't care about shell outputs (e.g. the vello render-to-pixel tests).
#[derive(Default)]
pub struct NoopPlatformApi;

impl PlatformApi for NoopPlatformApi {

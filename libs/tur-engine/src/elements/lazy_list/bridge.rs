...
pub(crate) fn fns() -> Vec<FnEntry> {
    vec![
        ...
        ("createLazyListController", 2, tur_create_lazy_list_controller as Ptr),
    ]
}
...
fn tur_lazy_list(
    ...
    let spec = super::LazyListView::from_js(&props, context).ok_or_else(|| {
        JsError::from(
            JsNativeError::typ().with_message("missing required prop for LazyListView"),
        )
    })?;
...
fn tur_create_lazy_list_controller(
    ...
    let _ = extract_ctx(args)?;
    let data = super::LazyListController::data_constructor(&JsValue::undefined(), &args[1..], context)?;
    Ok(super::LazyListController::from_data(data, context)?.upcast().clone().into())
}

...
pub(crate) fn extract_spans_from_js(value: &boa_engine::JsValue, context: &mut boa_engine::Context) -> Vec<SpanData> {
    ...
    for i in 0..len {
        let Ok(span_val) = arr.at(i as i64, context) else { continue };
        let Some(span_obj) = span_val.as_object() else { continue };
        ...
        let color = span_obj
            ...
            .ok()
            .and_then(|v| crate::core::bridge::color::extract_color(&v, context));

...
//!
//! Builder pattern: `Scrollbar(props)` returns a chainable builder
...
static TABLE: BuilderTable = BuilderTable {
    methods: &[
        M::new("color", color),
        M::new("trackColor", trackColor),
        M::new("thumbRadius", thumbRadius),
        M::new("thickness", thickness),
...
builder_factory!(tur_scrollbar_factory, tur_scrollbar, &TABLE);

pub fn fns() -> Vec<FnEntry> {
    vec![("Scrollbar", 2, tur_scrollbar_factory as Ptr)]
}

fn tur_scrollbar(_this: &JsValue, args: &[JsValue], context: &mut Context) -> JsResult<JsValue> {

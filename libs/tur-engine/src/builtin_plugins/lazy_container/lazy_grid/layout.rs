...
impl ElementLayout for LazyGridElement {
    fn perform_layout(
        ...
        // LazyGrid follows ListView/GridView semantics (unlike ScrollView):
        // it REQUIRES bounded constraints along its axis and fills them. An
        // unbounded axis collapses the viewport to zero — Flutter reports
        // this as a layout error; we log it once and degrade.
        if (!constraints.max_width.is_finite() || !constraints.max_height.is_finite())
            && !self.warned_unbounded
        {
            self.warned_unbounded = true;
            tracing::error!(
                "LazyGrid with unbounded constraints (width: {}, height: {}): \
        ...
        // previous pass would freeze the grid at zero (the grow-back could

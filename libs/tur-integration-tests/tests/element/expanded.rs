...
fn expanded_fills_remaining() {
    ...
    let (expanded_id, _inner_sb_id) = {
        ...
        assert_eq!(
            expanded.element.as_ref().unwrap().kind().as_str(),
            "tur_flex_item"
        );
        ...
        assert_eq!(
            inner_sb.element.as_ref().unwrap().kind().as_str(),
            "tur_container"
        );
...
fn expanded_multiple_share_evenly() {
    ...
    let (exp1_id, exp2_id) = {
        ...
        assert_eq!(
            exp1.element.as_ref().unwrap().kind().as_str(),
            "tur_flex_item"
        );
        assert_eq!(
            exp2.element.as_ref().unwrap().kind().as_str(),
            "tur_flex_item"
        );

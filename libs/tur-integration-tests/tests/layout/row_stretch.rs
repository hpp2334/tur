...
fn row_stretch_non_expanded_with_expanded_sibling() {
    ...
    let (sidebar_id, divider_id, expanded_id) = {
        ...
        let column = tree.get_element(root.children[0].as_element_id()).unwrap();
        let row = tree
            .get_element(column.children[0].as_element_id())
            .unwrap();
    ...
    let divider = rt.get_element(divider_id.as_element_id()).unwrap();

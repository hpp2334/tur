...
fn row_stretch_non_expanded_with_expanded_sibling() {
    ...
    let (sidebar_id, divider_id, expanded_id) = {
        ...
        let column = tree.get_element(ElementNodeId::new(root.children[0].as_u64())).unwrap();
        let row = tree.get_element(ElementNodeId::new(column.children[0].as_u64())).unwrap();
    ...
    let divider = rt.get_element(ElementNodeId::new(divider_id.as_u64())).unwrap();

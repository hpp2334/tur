...
//! free of compiler dependencies. Exposed to JS via the `builtin:demo-helper`
...
mod tests {
    ...
    fn generate_ast_finds_import() {
        let src = "import { Container, Text } from \"builtin:tur/std\";";
        let nodes = generate_ast(src).expect("parse should succeed");
        assert_eq!(nodes.len(), 1);
        match &nodes[0].kind {
            AstNodeKind::Import { source, specifiers } => {
                assert_eq!(source, "builtin:tur/std");
